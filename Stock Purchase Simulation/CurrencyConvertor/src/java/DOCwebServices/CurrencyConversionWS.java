...
public class CurrencyConversionWS {
    ...
    private double getUpdatedCurrencyRate(String currencyType) throws IOException, ParseException
    {
    ...
    public double GetConversionRate(String cur1, String cur2) throws IOException, ParseException {
        try {
            double rate1 = ExRate.valueOf(cur1).rateInGBP;
            if (getUpdatedCurrencyRate(cur1) != 0)
            {
                rate1 = getUpdatedCurrencyRate(cur1);
            }

            double rate2 = ExRate.valueOf(cur2).rateInGBP;
            if (getUpdatedCurrencyRate(cur2) != 0)
            {
                rate2 = getUpdatedCurrencyRate(cur2);
            }
            return rate1/rate2;
        }
